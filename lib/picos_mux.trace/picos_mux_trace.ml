...
let[@inline always][@specialise always] with_async_span continue discontinue ~parent ~__LINE__ name fn cont =
  let explicit_span = Trace.enter_manual_sub_span ~parent ~flavor:`Async ~__FILE__ ~__LINE__ name in
...
let rec run_fiber ~name ~fatal_exn_handler ~parent (current : Fiber.t) main =
  with_async_span continue discontinue ~parent ~__LINE__ name @@ fun ~parent ->
  let[@inline always][@specialise always] with_next_handler name ~__LINE__ fn =
    ...
    Some (with_async_span Effect.Deep.continue Effect.Deep.discontinue_with_backtrace ~parent ~__LINE__ name fn)
  in
  let effc :
...
let run_fiber ?(fatal_exn_handler = raise) current main =
  let parent =
    Trace.enter_manual_toplevel_span ~flavor:`Async ~__LINE__ ~__FILE__ "run_fiber"
  in
